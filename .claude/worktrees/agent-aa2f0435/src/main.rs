...
async fn main() -> Result<()> {
    ...
    if config.bootstrap.enabled {
        ...
        if let (Some(url), Some(token)) = (&config.bootstrap.url, &config.bootstrap.token) {
            // Read discovery name from DB, fall back to instance_name
            let discovery_name: String = sqlx::query_scalar::<_, serde_json::Value>(
                "SELECT value FROM app_config WHERE key = 'discovery_name'",
            )
