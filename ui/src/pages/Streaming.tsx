...
type SourceFilter = 'all' | 'stackarr' | 'plex'
        ...
        <div className="flex items-center gap-2">
          ...
          {plexEnabled && streamingEnabled && (
            <div className="flex rounded-lg bg-slate-800 p-0.5 text-xs">
              {(['all', 'stackarr', 'plex'] as const).map((f) => (
                <button
                  key={f}
        ...
        <div className="space-y-2">
          {filtered.map((session) => (
            <SessionCard
              ...
              session={session}
              advanced={advanced}
              onStop={session.source === 'stackarr' ? () => stopSession.mutate(session.id) : undefined}
