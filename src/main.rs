...
use ngms_indexer::IndexerManager;
use ngms_scheduler::Scheduler;
use ngms_web::AppState;

#[derive(Parser, Debug)]
#[command(name = "ngms", version, about = "NGMS media management server")]
struct Cli {
    /// Path to the configuration file
    #[arg(short, long, default_value = "/config/ngms.toml", env = "NGMS_CONFIG")]
    config: PathBuf,

    /// Override bind address
    #[arg(long, env = "NGMS_BIND")]
    bind: Option<String>,

    /// Override port
    #[arg(long, env = "NGMS_PORT")]
    port: Option<u16>,

    /// Override database URL
    #[arg(long, env = "NGMS_DATABASE_URL")]
    database_url: Option<String>,

    /// Log level (trace, debug, info, warn, error)
    #[arg(long, env = "NGMS_LOG_LEVEL", default_value = "info")]
    log_level: String,
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Some(Commands::Migrate {
            ...
            let path_mappings: Vec<ngms_migrate::PathMapping> = path_maps
                .iter()
                .filter_map(|s| {
                    let (from, to) = s.split_once('=')?;
                    Some(ngms_migrate::PathMapping {
                        from: from.to_string(),
                        to: to.to_string(),
                    })
                })
            ...
            let report = ngms_migrate::run_migration(
                db.pool(),
                sonarr.as_deref(),
                radarr.as_deref(),
                prowlarr.as_deref(),
    ...
    let indexarr_client = if config.indexarr.enabled {
        match &config.indexarr.api_key {
            Some(api_key) if !api_key.is_empty() => {
                ...
                let client = ngms_indexer::IndexarrClient::new(
                    &config.indexarr.url,
                    api_key,
                );
                Some(Arc::new(client))
            ...
            Ok(rows) => {
                for (id, name, base_url, api_key, protocol, enabled, priority) in rows {
                    let proto = match protocol.as_str() {
                        "torrent" => ngms_indexer::newznab::Protocol::Torrent,
                        _ => ngms_indexer::newznab::Protocol::Usenet,
                    };
        ...
        .await
        {
            Ok(rows) => {
                for (id, name, client_type, _protocol, cfg, _enabled, priority) in rows {
                    match ngms_download::build_from_config(&client_type, &cfg) {
                        Ok(client) => {
    ...
    let rate_limiter = Some(ngms_web::middleware::create_rate_limiter(50));

    // 14. Initialize Cardigann engine (load bundled definitions)
    let cardigann_engine = {
        let definitions_dir = std::path::Path::new("crates/ngms-cardigann/definitions");
        let mut engine = ngms_cardigann::CardigannEngine::new(definitions_dir);
    ...
    let stream_session_manager = if config.streaming.enabled {
        ...
        let ffmpeg_paths = ngms_stream::ensure_ffmpeg(
        ...
        let detected_accel = ngms_stream::probe_hwaccel(
            &ffmpeg_paths.ffmpeg,
            &streaming_config.hwaccel,
        ).await;
        tracing::info!(accel = %detected_accel, "streaming encoder: {detected_accel}");

        let mgr = Arc::new(ngms_stream::SessionManager::new(
            streaming_config,
            detected_accel,
            db.pool().clone(),
        ));
    ...
    let tmdb_client = {
        let key = std::env::var("NGMS_TMDB_API_KEY")
            .ok()
            .filter(|k| !k.is_empty());
        let key = match key {
            Some(k) => Some(k),
        ...
        key.map(|k| {
            ...
            Arc::new(ngms_metadata::TmdbClient::new(k))
        })
    };
    ...
    let indexarr_available = std::env::var("NGMS_INDEXARR_ENABLED")
    ...
    ngms_web::run(&listen_addr, state).await?;

    tracing::info!("NGMS shut down cleanly");
    Ok(())
}
