...
use ngms_core::models::{Episode, Movie, Series};
...
impl SeriesService {
    ...
    pub async fn create(&self, input: CreateSeriesInput) -> Result<Series> {
        let clean = ngms_parser::clean_title(&input.title);
    ...
    pub async fn update(&self, id: i64, input: UpdateSeriesInput) -> Result<Series> {
        ...
        let clean = ngms_parser::clean_title(&title);
...
mod tests {
    use super::*;
    use ngms_core::test_helpers::{TestDb, seed_quality_profile, seed_media_library_folder, seed_series, seed_episode};
