...
PostgreSQL 17 is required. SQLite is only used for reading *arr migration databases (rusqlite in `ngms-migrate`).
...
The standalone `ngms-bootstrap` binary uses a separate SQLite database (not PostgreSQL) for its own persistence. This is the only component that writes to SQLite.
...
| `server_id` | TEXT | UUID of the registered NGMS server |
...
All models are defined in `crates/ngms-core/src/models/user.rs` and derive `FromRow`, `Serialize`, `Deserialize`.
...
Defined in `crates/ngms-import/src/recycle_bin.rs`:
...
3. Add corresponding model structs to `ngms-core/src/models.rs`
...
Integration tests use `ngms_core::test_helpers::TestDb` (behind `testing` feature flag):
