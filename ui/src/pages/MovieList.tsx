...
import { Plus, Search, Film, X, Loader2, CheckCircle, Pencil, Check, SlidersHorizontal } from 'lucide-react'
...
export default function MovieList() {
  ...
  const [filters, setFilters] = useState<MovieFilters>({ status: '', qualityProfileId: '', monitored: '', hasFile: '' })
  ...
  const allStatuses = useMemo(
    () => [...new Set((movies ?? []).map((m) => m.status).filter(Boolean))].sort(),
    [movies],
  )
  ...
  const clearFilters = () => setFilters({ status: '', qualityProfileId: '', monitored: '', hasFile: '' })
  ...
  const filtered = useMemo(() => {
    return (movies ?? []).filter((m) => {
      ...
      if (filters.qualityProfileId !== '' && m.qualityProfileId !== filters.qualityProfileId) return false
      if (filters.monitored !== '') {
        if (m.monitored !== (filters.monitored === 'true')) return false
      }
      if (filters.hasFile !== '') {
        if (m.hasFile !== (filters.hasFile === 'true')) return false
      }
      return true
    })
      ...
      {view === 'library' && showFilters && (
        <div className="mb-4 flex flex-wrap items-center gap-3 rounded-lg border border-slate-700 bg-slate-800/50 px-4 py-3">
          ...
          <select
            ...
            onChange={(e) => setFilters((f) => ({ ...f, hasFile: e.target.value as '' | 'true' | 'false' }))}
            className="rounded-lg border border-slate-600 bg-slate-700 px-3 py-1.5 text-sm text-white focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
          >
          ...
          <span className="ml-auto text-xs text-slate-500">
            {filtered.length} of {movies?.length ?? 0} movies
          </span>
        </div>
      )}
          ...
          {!isLoading && !error && filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 text-slate-400">
              ...
              {filter || activeFilterCount > 0 ? (
                <>
                  <p className="mb-2">No movies match the current filters</p>
