...
//!
//! Requires a running PostgreSQL instance (e.g. `docker compose -f docker/docker-compose.dev.yml up -d`).
...
const DEFAULT_TEST_URL: &str = "postgresql://ngms:ngms@localhost:5433/postgres";
...
impl TestDb {
    ...
    pub async fn new() -> Self {
        let base = base_url();
        let name = format!("ngms_test_{}", uuid::Uuid::new_v4().simple());
        ...
        let admin_pool = PgPoolOptions::new()
            ...
            .connect(&base)
            .await
            .expect("failed to connect to admin postgres — is docker compose running?");
