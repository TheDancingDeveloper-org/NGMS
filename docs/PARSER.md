...
`stackarr-parser` is a zero-dependency (no DB, no async) crate that extracts structured metadata from media release names like `"Show.Name.S01E02.720p.HDTV.x264-GROUP"`.

## Entry Point

```rust
use stackarr_parser::{parse_release, ParsedRelease, clean_title};
...
| Crate | Uses |
|-------|------|
| `stackarr-media` | `clean_title()` on create/update for matching |
| `stackarr-import` | `parse_release()` on imported files for quality/language |
| `stackarr-indexer` | `parse_release()` on search results for quality info |
| `stackarr-quality` | `Quality` enum for profile matching |
| `stackarr-migrate` | `clean_title()` for *arr data normalization |
