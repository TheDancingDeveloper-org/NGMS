...
/// Radarr uses different quality IDs from Sonarr/NGMS.
/// This maps Radarr quality IDs to NGMS IDs (which match Sonarr).
fn radarr_quality_id_to_ngms(radarr_id: i64) -> i64 {
    match radarr_id {
        0 => 0,   // Unknown
        1 => 1,   // SDTV
        2 => 2,   // DVD
...
/// Recursively normalize Radarr quality IDs in profile items JSON to NGMS IDs.
        ...
        match q {
            JsonValue::Number(n) => {
                if let Some(id) = n.as_i64() {
                    let mapped = radarr_quality_id_to_ngms(id);
    ...
    if let Some(r) = radarr {
        for p in &r.quality_profiles {
            let raw_items: JsonValue =
                serde_json::from_str(&p.items).unwrap_or(JsonValue::Array(vec![]));
            // Remap Radarr quality IDs to NGMS/Sonarr numbering
            let items = normalize_radarr_quality_ids(&raw_items);
            let cutoff = radarr_quality_id_to_ngms(p.cutoff as i64) as i32;
...
impl MigrationWriter {
    ...
    pub async fn write_all(&self, data: MigrationData) -> Result<MigrationReport> {
        ...
        //     confusing to the end user (events they never triggered in NGMS).
