...
use ngms_core::models::quality::QualityProfile;
use ngms_quality::{is_quality_allowed, parser_quality_to_num, quality_name};
...
fn extract_quality_num(quality_json: &serde_json::Value) -> i32 {
    ...
    if let Some(quality_val) = quality_json.get("quality") {
        if let Ok(q) = serde_json::from_value::<ngms_parser::Quality>(quality_val.clone()) {
