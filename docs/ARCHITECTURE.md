...
NGMS is a monolithic Rust binary that embeds a web server, background scheduler, and optional download engines (torrent + usenet). It connects to PostgreSQL for persistence and serves a React SPA for the UI.
...
19. **Init shared TMDB client** — Load API key from `NGMS_TMDB_API_KEY` env var or `app_config` DB table. Creates a rate-limited + cached `TmdbClient` if a key is available.
...
```
src/main.rs
├── ngms-core         (config, db, models, errors)
├── ngms-web          (routes, middleware)
│   ├── ngms-media    (series/movie CRUD)
│   │   ├── ngms-parser
│   │   └── ngms-metadata
│   ├── ngms-quality  (profiles, scoring)
│   │   └── ngms-parser
│   ├── ngms-indexer  (search, RSS)
│   │   ├── ngms-parser
│   │   └── ngms-cardigann
│   ├── ngms-download (client management)
│   ├── ngms-import   (disk scan, rename)
│   │   ├── ngms-parser
│   │   ├── ngms-media
│   │   └── ngms-quality
│   ├── ngms-notify   (webhooks, Discord)
│   ├── ngms-migrate  (Sonarr/Radarr import)
│   │   └── ngms-parser
│   ├── ngms-plex     (Plex API, sync)
│   │   └── ngms-metadata
│   └── ngms-stream   (video streaming, HLS, ffmpeg)
├── ngms-scheduler    (background tasks)
│   ├── ngms-media
│   ├── ngms-metadata
│   ├── ngms-indexer
│   ├── ngms-download
...
│   ├── ngms-quality
│   └── ngms-plex
├── ngms-bootstrap    (standalone discovery node)
├── ngms-cardigann-parity (QA tool: Prowlarr parity testing)
...
NGMS features are gated by `EnabledModules`:
...
NGMS includes a Cardigann engine (`ngms-cardigann`) compatible with Prowlarr's YAML indexer definitions. This allows adding indexers from a catalog of YAML definitions rather than requiring hardcoded client implementations.
...
- A separate QA binary (`ngms-cardigann-parity`) validates search result parity against Prowlarr.

## Streaming Engine (ngms-stream)
...
NGMS uses a setup-based first-boot flow instead of auto-generated credentials:
...
## Remote Access (ngms-bootstrap)

The bootstrap system enables remote server discovery, client pairing, and server name resolution:

- A standalone `ngms-bootstrap` binary acts as a discovery node with **SQLite persistence** (`server_names` + `pending_claims` tables).
- NGMS instances register a human-readable server name with the bootstrap service.
