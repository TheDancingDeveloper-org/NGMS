...
NGMS is configured via a TOML file, environment variables, and CLI flags. CLI/env overrides take precedence over the config file.

## Config File

Default location: `ngms.toml` in the working directory. Override with `--config PATH` or `NGMS_CONFIG`.
...
```toml
[general]
instance_name = "NGMS"      # Display name in UI
...
All env vars are prefixed with `NGMS_`:

| Variable | Maps To | Example |
|----------|---------|---------|
| `NGMS_CONFIG` | CLI `--config` | `/config/ngms.toml` |
| `NGMS_BIND` | `general.bind_addr` | `0.0.0.0` |
| `NGMS_PORT` | `general.port` | `8989` |
| `NGMS_DATABASE_URL` | `database.url` | `postgresql://...` |
| `NGMS_LOG_LEVEL` | `general.log_level` | `debug` |
...
```
USAGE:
    ngms [OPTIONS] [SUBCOMMAND]

OPTIONS:
    --config <PATH>          Config file path [env: NGMS_CONFIG]
    --bind <ADDR>            Bind address [env: NGMS_BIND]
    --port <PORT>            Port number [env: NGMS_PORT]
    --database-url <URL>     Database URL [env: NGMS_DATABASE_URL]
    --log-level <LEVEL>      Log level [env: NGMS_LOG_LEVEL]
...
| `testing` | ngms-core | Expose TestDb helper for integration tests |
