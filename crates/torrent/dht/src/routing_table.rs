...
mod tests {
    ...
    fn test_routing_table_respects_bucket_size() {
        ...
        for i in 0u8..8 {
            ...
            let addr: SocketAddr = SocketAddrV4::new(Ipv4Addr::new(10, 0, 0, i + 1), 6881).into();
    ...
    fn test_routing_table_splitting() {
        ...
        for i in 1u8..=9 {
            ...
            let id = Id20::new(data);
            let addr: SocketAddr = SocketAddrV4::new(Ipv4Addr::new(10, 0, 0, i), 6881).into();
            rt.add_node(id, addr);
        }
        // All 9 should be added because the bucket containing our id will split.
        assert!(rt.len() >= 9, "expected at least 9 nodes, got {}", rt.len());
    }
    ...
    fn test_replace_bad_node() {
        ...
        // Verify it's bad.
        {
            ...
            assert!(matches!(
                node.status(Instant::now()),
                super::NodeStatus::Bad
            ));
        }
