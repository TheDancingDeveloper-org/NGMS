...
use serde::Serialize;
use ngms_plex::types::PlexServer;
use ngms_plex::PlexApi;
use ngms_stream::types::TranscodeRequest;
...
async fn stream_info(
    ...
    match ngms_stream::ffprobe::probe(&config.ffprobe_path, &file_path).await {
        Ok(info) => {
...
async fn stream_direct(
    ...
    match ngms_stream::direct::serve_file(&file_path, range_header).await {
        Ok(resp) => {
        ...
        Err(ngms_stream::StreamError::NotFound(_)) => {
            (
                StatusCode::NOT_FOUND,
        ...
        Err(ngms_stream::StreamError::InvalidRange(msg)) => {
            (
...
async fn start_transcode(
    ...
    match result {
        ...
        Err(ngms_stream::StreamError::MaxSessions) => (
...
fn select_rendition_tiers(
    tiers: &[ngms_core::config::QualityTierConfig],
    max: usize,
) -> Vec<ngms_core::config::QualityTierConfig> {
    if tiers.len() <= max {
        return tiers.to_vec();
    }
...
async fn hls_playlist(
    ...
    match ngms_stream::hls::read_playlist(&session_dir, &api_prefix).await {
        Ok(playlist) => {
...
async fn hls_segment(
    ...
    if let Err(e) =
        ngms_stream::hls::wait_for_segment(&session_dir, &segment, Duration::from_secs(30))
            .await
    {
    ...
    match ngms_stream::hls::read_segment(&session_dir, &segment).await {
        Ok(data) => {
            let mut headers = HeaderMap::new();
...
async fn stream_subtitle(
    ...
    if !output_path.exists() {
        if let Err(e) = ngms_stream::subtitle::extract_to_webvtt(
            &config.ffmpeg_path,
            &file_path,
            track_index,
            &output_path,
...
async fn hls_sub_playlist(
    ...
    match ngms_stream::hls::read_sub_playlist(&rendition_dir, &api_prefix).await {
        Ok(playlist) => {
            let mut headers = HeaderMap::new();
...
async fn hls_sub_segment(
    ...
    // Wait for segment
    if let Err(e) = ngms_stream::hls::wait_for_segment(&rendition_dir, &segment, Duration::from_secs(30)).await {
        tracing::warn!(segment = %segment, rendition = %rendition, error = %e, "sub-segment wait timed out");
        return (StatusCode::NOT_FOUND, "segment not ready").into_response();
    }

    match ngms_stream::hls::read_segment(&rendition_dir, &segment).await {
        Ok(data) => {
            let mut headers = HeaderMap::new();
            headers.insert("content-type", HeaderValue::from_static("video/mp2t"));
            (StatusCode::OK, headers, data).into_response()
...
async fn quality_tiers(
    ...
    let tiers: Vec<ngms_stream::types::QualityTier> = config
        .streaming
        .quality_tiers
        .iter()
        .filter(|t| t.max_height <= source_height)
        .map(|t| ngms_stream::types::QualityTier {
            name: t.name.clone(),
    ...
    let mut result = vec![ngms_stream::types::QualityTier {
