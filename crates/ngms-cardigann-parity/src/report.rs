...
pub fn write_report(results: &[ParityResult], path: &Path) -> Result<()> {
    ...
    let errored = results
        .iter()
        .filter(|r| r.prowlarr_error.is_some() || r.ngms_error.is_some())
        .count();
    ...
    md.push_str("| Indexer | Query | Prowlarr | NGMS | Match | Parity | Issues |\n");
    md.push_str("|---------|-------|----------|----------|-------|--------|--------|\n");

    for r in results {
        let issues = if let Some(ref e) = r.prowlarr_error {
            format!("Prowlarr: {}", truncate(e, 40))
        } else if let Some(ref e) = r.ngms_error {
            format!("NGMS: {}", truncate(e, 40))
    ...
    if !poor_results.is_empty() {
        ...
        for r in &poor_results {
            ...
            if !r.ngms_only.is_empty() {
                md.push_str("\n**NGMS only:**\n");
                for t in r.ngms_only.iter().take(10) {
                    md.push_str(&format!("- {t}\n"));
                }
            }
            md.push('\n');
...
pub fn print_summary(results: &[ParityResult]) {
    ...
    for r in sorted.iter().take(5) {
        println!(
            "  {:<30} \"{:<10}\"  {:.0}%  (P:{} S:{})",
            r.indexer_name, r.query, r.parity_pct, r.prowlarr_count, r.ngms_count
        );
    }
}
