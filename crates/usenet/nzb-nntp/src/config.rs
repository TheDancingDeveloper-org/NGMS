//! NNTP server configuration and article model.
//!
//! These types were originally in nzb-core but live here so that nzb-nntp has
//! zero dependency on nzb-core.  nzb-core re-exports them so downstream code
//! is unaffected.
...
mod tests {
    ...
    fn test_article_serde_roundtrip() {
        ...
        assert_eq!(restored.segment_number, original.segment_number);
        assert_eq!(restored.bytes, original.bytes);
        assert_eq!(restored.downloaded, original.downloaded);
        assert_eq!(restored.tried_servers, original.tried_servers);
        assert_eq!(restored.tries, original.tries);
    }
}
