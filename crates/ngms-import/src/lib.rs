...
async fn process_single_file(
    ...
    let filename = file
        ...
        .and_then(|f| f.to_str())
        .unwrap_or("");
    let parsed = ngms_parser::parse_release(filename);
...
async fn import_series_file(
    ctx: &ImportContext,
    file: &LocalFile,
    parsed: &ngms_parser::ParsedRelease,
    result: &mut ImportResult,
) -> Result<()> {
    let pool = &ctx.pool;
    ...
    // ── Upgrade check ───────────────────────────────────────────────────
    let new_quality_num =
        ngms_quality::parser_quality_to_num(parsed.quality.quality.clone());
    ...
    match upgrade_result {
        ...
        upgrade::UpgradeCheckResult::Upgrade {
            ...
            let delete_data = serde_json::json!({
                ...
                "replaced_by_quality": ngms_quality::quality_name(new_quality_num),
            });
            sqlx::query(
            ...
            tracing::info!(
                ...
                new_quality = ngms_quality::quality_name(new_quality_num),
...
async fn scan_series(pool: &PgPool, root_path: &Path) -> Result<DiskScanResult> {
        ...
        .filter_map(|e| e.ok())
    {
        ...
        let clean_dir = ngms_parser::clean_title(&series_dir_name);
...
async fn scan_movies(pool: &PgPool, root_path: &Path) -> Result<DiskScanResult> {
        ...
        .filter_map(|e| e.ok())
    {
        ...
        let clean_dir = ngms_parser::clean_title(&movie_dir_name);
