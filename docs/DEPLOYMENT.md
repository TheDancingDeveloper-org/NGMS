...
| `docker/docker-compose.prod.yml` | Production example — read-only media mounts, memory limits |
...
services:
  stackarr:
    ...
    volumes:
      ...
      - /mnt/downloads/usenet:/downloads/usenet:ro
      - /mnt/downloads/torrent:/downloads/torrent:ro
    deploy:
      resources:
        limits:
          memory: 4096m
  ...
  postgres:
    ...
    volumes:
      - stackarr_pgdata:/var/lib/postgresql/data
    healthcheck:
...
- Memory limited to 4GB (adjust to your host)
...
CI runs on a self-hosted GitHub Actions runner tagged `[self-hosted]`. Uses GCC 12 for compilation (CC/CXX env vars set).
