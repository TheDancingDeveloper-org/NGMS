...
pub(crate) fn resolve_quality(q: &serde_json::Value) -> serde_json::Value {
    if let Some(obj) = q.as_object() {
        if let Some(num) = obj.get("quality").and_then(|v| v.as_i64()) {
            let name = ngms_quality::quality_name(num as i32);
...
pub(crate) fn resolve_media_file_quality(mut file: ngms_core::models::media::MediaFile) -> ngms_core::models::media::MediaFile {
