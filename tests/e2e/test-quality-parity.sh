...
#
# Compares release search scoring between NGMS (after importing
...
#   SONARR_URL      — Sonarr base URL (default: https://sonarr.example.com/sonarr)
#   RADARR_URL      — Radarr base URL (default: https://radarr.example.com)
...
PROJECT="ngms-quality"
BASE_URL="http://localhost:9214"
export MEDIA_BASE="${MEDIA_BASE:-/tmp/ngms-quality-test}"

SONARR_URL="${SONARR_URL:-https://sonarr.example.com/sonarr}"
RADARR_URL="${RADARR_URL:-https://radarr.example.com}"
NODE_B="${NODE_B:-YOUR_SERVER_IP}"

# Auto-discover API keys from production server if not provided
if [[ -z "${SONARR_API_KEY:-}" ]]; then
    log "SONARR_API_KEY not set — discovering from production server ($NODE_B)..."
    SONARR_API_KEY=$(ssh -o ConnectTimeout=5 -o BatchMode=yes "deploy_user@${NODE_B}" \
        "docker exec sonarrv4 cat /config/config.xml 2>/dev/null | grep -oP '<ApiKey>\K[^<]+'" 2>/dev/null) || true
    if [[ -z "$SONARR_API_KEY" ]]; then
        echo "ERROR: Could not discover SONARR_API_KEY. Set it manually or ensure SSH to production server works."
        exit 1
    fi
...
if [[ -z "${RADARR_API_KEY:-}" ]]; then
    log "RADARR_API_KEY not set — discovering from production server ($NODE_B)..."
    RADARR_API_KEY=$(ssh -o ConnectTimeout=5 -o BatchMode=yes "deploy_user@${NODE_B}" \
        "docker exec radarr cat /config/config.xml 2>/dev/null | grep -oP '<ApiKey>\K[^<]+'" 2>/dev/null) || true
    if [[ -z "$RADARR_API_KEY" ]]; then
        echo "ERROR: Could not discover RADARR_API_KEY. Set it manually or ensure SSH to production server works."
        exit 1
    fi
...
# Args: $1=label $2=reference_json_file $3=ngms_json_file $4=media_type
compare_releases() {
    local label="$1"
    local ref_file="$2"
    local arz_file="$3"
    ...
    log "  $label: reference=$ref_count releases, NGMS=$arz_count releases"
    ...
    # --- Check 1: NGMS returned results ---
    if [[ "$arz_count" -gt 0 ]]; then
        ok "$label — NGMS returned $arz_count releases"
    else
        fail "$label — NGMS returned 0 releases (expected ~$ref_count)"
        return
    fi

    # --- Check 2: Match titles between ref and NGMS ---
    ...
    # For each title in the reference, find it in NGMS and compare
    while IFS= read -r title; do
        ...
        # NGMS wraps in DownloadDecision { release: { title: ... }, approved, rejections }
        ...
        # NGMS parses quality from title via ngms-parser; the release endpoint
    ...
    log "    Only in NGMS: $only_in_arz"

    # Pass/fail thresholds
    if [[ "$total_checked" -gt 0 ]]; then
        ok "$label — $total_checked titles compared"
    else
        fail "$label — no titles matched between reference and NGMS"
    fi
...
if [[ "${SKIP_SETUP:-0}" != "1" ]]; then
    section "Setup — Start NGMS Stack"
    ...
    SETUP_PAYLOAD=$(cat <<'JSON'
{
  ...
  "instanceName": "NGMS-Test-Quality"
}
JSON
)
    ...
    response=$(curl -s -S -w '\n%{http_code}' -X POST \
        ...
        -F 'path_mappings=[{"from":"/path/to/TV1","to":"/media/TV1"},{"from":"/path/to/Movies2","to":"/media/Movies2"}]' \
    ...
    log "NGMS has $arz_profiles quality profiles"

    # Dump profiles for reference
    echo "$API_BODY" | jq '.' > "$RESULTS_DIR/ngms_quality_profiles.json"
    ok "Quality profiles saved to $RESULTS_DIR/ngms_quality_profiles.json"

    # Restart NGMS so imported indexers are loaded into memory
    section "Setup — Restart NGMS"
    log "Restarting NGMS to load imported indexers..."
    docker compose -f "$COMPOSE_FILE" -p "$PROJECT" restart ngms 2>&1 | tail -3
    wait_for_health 60
...
# --- NGMS IDs (for context-aware searches) ---
...
log "NGMS Outlander series ID: ${ARZ_OUTLANDER_ID:-none}"
log "NGMS NCIS series ID: ${ARZ_NCIS_ID:-none}"
...
if [[ -n "${ARZ_OUTLANDER_ID:-}" && "$ARZ_OUTLANDER_ID" != "null" ]]; then
    ...
    log "NGMS Outlander S08E01 episode ID: ${ARZ_OUTLANDER_EP_ID:-none}"
fi
...
if [[ -n "${ARZ_NCIS_ID:-}" && "$ARZ_NCIS_ID" != "null" ]]; then
    ...
    log "NGMS NCIS S23E01 episode ID: ${ARZ_NCIS_EP_ID:-none}"
fi
...
log "NGMS Anaconda movie ID: ${ARZ_ANACONDA_ID:-none}"
log "NGMS GLHF movie ID: ${ARZ_GLHF_ID:-none}"
...
# Build NGMS search URL with context params for decision engine
...
log "Searching NGMS for Outlander S08E01..."
log "  params: $ARZ_OUTLANDER_PARAMS"
api GET "/api/v1/release?${ARZ_OUTLANDER_PARAMS}"
if [[ "$API_CODE" == "200" ]]; then
    echo "$API_BODY" | jq '.' > "$RESULTS_DIR/ngms_outlander_s08e01.json"
    arz_count=$(jq 'length' "$RESULTS_DIR/ngms_outlander_s08e01.json")
    ok "NGMS returned $arz_count releases"
else
    fail "NGMS search failed (HTTP $API_CODE)"
    echo "$API_BODY" | head -5
fi

compare_releases "Outlander S08E01" \
    "$RESULTS_DIR/sonarr_outlander_s08e01.json" \
    "$RESULTS_DIR/ngms_outlander_s08e01.json" \
    "series"
...
log "Searching NGMS for NCIS S23E01..."
log "  params: $ARZ_NCIS_PARAMS"
api GET "/api/v1/release?${ARZ_NCIS_PARAMS}"
if [[ "$API_CODE" == "200" ]]; then
    echo "$API_BODY" | jq '.' > "$RESULTS_DIR/ngms_ncis_s23e01.json"
    arz_count=$(jq 'length' "$RESULTS_DIR/ngms_ncis_s23e01.json")
    ok "NGMS returned $arz_count releases"
else
    fail "NGMS search failed (HTTP $API_CODE)"
fi
...
compare_releases "NCIS S23E01" \
    ...
    "$RESULTS_DIR/ngms_ncis_s23e01.json" \
    "series"
...
log "Searching NGMS for Anaconda 2025..."
log "  params: $ARZ_ANACONDA_PARAMS"
api GET "/api/v1/release?${ARZ_ANACONDA_PARAMS}"
if [[ "$API_CODE" == "200" ]]; then
    echo "$API_BODY" | jq '.' > "$RESULTS_DIR/ngms_anaconda_2025.json"
    arz_count=$(jq 'length' "$RESULTS_DIR/ngms_anaconda_2025.json")
    ok "NGMS returned $arz_count releases"
else
    fail "NGMS search failed (HTTP $API_CODE)"
fi
...
compare_releases "Anaconda 2025" \
    ...
    "$RESULTS_DIR/ngms_anaconda_2025.json" \
    "movie"
...
log "Searching NGMS for Good Luck Have Fun Don't Die..."
log "  params: $ARZ_GLHF_PARAMS"
api GET "/api/v1/release?${ARZ_GLHF_PARAMS}"
if [[ "$API_CODE" == "200" ]]; then
    echo "$API_BODY" | jq '.' > "$RESULTS_DIR/ngms_glhf.json"
    arz_count=$(jq 'length' "$RESULTS_DIR/ngms_glhf.json")
    ok "NGMS returned $arz_count releases"
else
    fail "NGMS search failed (HTTP $API_CODE)"
fi
...
for search_name in outlander_s08e01 ncis_s23e01 anaconda_2025 glhf; do
    ...
    arz_file="$RESULTS_DIR/ngms_${search_name}.json"
    ...
    # NGMS top 10
    printf "  %-60s  %-20s  %s  %s\n" "TITLE (NGMS)" "REJECTIONS" "APPROVED" "N_REJ"
    printf "  %-60s  %-20s  %s  %s\n" "$(printf '%.0s─' {1..60})" "$(printf '%.0s─' {1..20})" "────────" "────────"
    jq -r '.[0:10][] | [
