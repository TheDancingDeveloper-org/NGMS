...
# 2. Run backend
cargo run -- --config ngms.toml --database-url postgresql://ngms:ngms@localhost:5433/ngms
...
cargo test -p ngms-import
cargo test -p ngms-notify

# Integration tests (requires Postgres on :5433)
cargo test -p ngms-migrate -- --ignored

# Single test
cargo test -p ngms-parser test_parse_standard_release
```
...
async fn test_full_migration() {
    let db = TestDb::new("postgresql://ngms:ngms@localhost:5433/ngms").await;
    // ...
}
```
...
1. **Route file**: Add handler in `crates/ngms-web/src/routes/your_route.rs`
2. **Router function**: Export `pub fn router() -> Router<Arc<AppState>>`
3. **Register**: Add `.merge(routes::your_route::router())` in `crates/ngms-web/src/lib.rs`
4. **Service** (if needed): Add to appropriate crate (e.g., `ngms-media`)
5. **Models** (if new): Add to `ngms-core/src/models.rs`

### Adding a New Crate

1. Create `crates/ngms-yourname/Cargo.toml` and `src/lib.rs`
2. Add to workspace members in root `Cargo.toml`
3. Add workspace dependency: `ngms-yourname = { path = "crates/ngms-yourname" }`
4. Import in consuming crates

### Adding a Database Migration

1. Create `migrations/NNN_description.sql`
2. Add model structs in `ngms-core/src/models.rs` with `FromRow`, `Serialize`, `Deserialize`
...
```bash
# Verbose logging
NGMS_LOG_LEVEL=debug cargo run -- --config ngms.toml

# Trace-level (very verbose)
NGMS_LOG_LEVEL=trace cargo run -- --config ngms.toml

# Single crate trace
RUST_LOG=ngms_parser=trace cargo run -- --config ngms.toml
```
...
psql postgresql://ngms:ngms@localhost:5433/ngms
