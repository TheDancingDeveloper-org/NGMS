...
**Dependencies**: All other ngms crates, axum, tower-http, utoipa (OpenAPI)
...
**Behavior**:
- Auto-computes `clean_title` via `ngms_parser::clean_title()` on create/update.
...
**Dependencies**: ngms-parser, ngms-metadata, sqlx
...
**Dependencies**: ngms-parser, ngms-core, sqlx, serde, governor
...
- `IndexarrClient` — NGMS's sidecar indexer client
...
**Behavior**: Parallel search across all enabled indexers. Parses XML responses (Newznab) or JSON (Indexarr). Cardigann indexers are built dynamically from YAML definitions via `ngms-cardigann`. Supports RSS feed polling for new releases.

**Dependencies**: ngms-parser, ngms-cardigann, sqlx, reqwest, quick-xml, futures
...
**Dependencies**: ngms-parser, ngms-media, ngms-quality, sqlx, walkdir, tokio
...
**Purpose**: Import data from Sonarr/Radarr/Prowlarr SQLite databases into NGMS's PostgreSQL.

**Usage**: `ngms migrate --sonarr path.db --radarr path.db --prowlarr path.db [--dry-run]`
...
**Dependencies**: ngms-parser, sqlx, rusqlite, serde, chrono, anyhow
...
- `PlexScanner` — scans and syncs Plex library to NGMS
...
**Dependencies**: ngms-metadata, sqlx, reqwest, regex
...
**Dependencies**: ngms-core, tokio, tokio-util, sqlx, uuid, dashmap, bytes, mime_guess
...
**Dependencies**: serde_yaml, scraper, indexmap, regex, reqwest, chrono, url (no ngms deps)

---

### ngms-cardigann-parity
...
**Dependencies**: ngms-cardigann, ngms-indexer, reqwest, tokio
...
- Used by NGMS instances to enable remote access via the bootstrap protocol

**Key modules**:
- `db.rs` — SQLite persistence layer (rusqlite): `init_db()`, `register_server_name()`, `resolve_name()`, `store_claim()`, `redeem_claim()`

**Dependencies**: axum, tokio, clap, serde_json, rusqlite, uuid, toml, bip39 (no ngms deps)
