//! Compare Prowlarr search results with NGMS Cardigann engine results.

use std::collections::HashMap;
use std::path::Path;

use anyhow::{Context, Result};
use ngms_cardigann::search::{CardigannIndexer, CardigannRelease, SearchQuery, SearchType};
...
fn compare_results(
    ...
    ngms: Result<Vec<CardigannRelease>>,
) -> ParityResult {
    ...
    let (ngms_releases, ngms_error) = match ngms {
        Ok(r) => (r, None),
        Err(e) => (Vec::new(), Some(e.to_string())),
    };
    ...
    let ngms_titles: HashMap<String, &CardigannRelease> =
        ngms_releases
            .iter()
    ...
    let mut ngms_only = Vec::new();

    for (norm_title, pr) in &prowlarr_titles {
        if let Some(sr) = ngms_titles.get(norm_title) {
            matched += 1;
    ...
    for (norm_title, sr) in &ngms_titles {
        if !prowlarr_titles.contains_key(norm_title) {
            ngms_only.push(sr.title.clone());
        }
    }

    let total = prowlarr_releases.len().max(ngms_releases.len());
