...
async fn base_url(pool: &sqlx::PgPool, config: &ngms_core::config::AppConfig) -> String {
...
fn quality_label(quality: &serde_json::Value) -> String {
    if let Some(num) = quality.get("quality").and_then(|v| v.as_i64()) {
        ngms_quality::quality_name(num as i32).to_string()
...
async fn manifest(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    Json(Manifest {
        ...
        name: "NGMS",
        description: "Stream your NGMS media library in Stremio",
