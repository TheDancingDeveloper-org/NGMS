...
struct StatusResponse {
    ...
    /// Whether the Indexarr container is deployed (NGMS_INDEXARR_ENABLED env var).
...
async fn init_setup(
    ...
    let recovery_phrase = if streaming_enabled {
        match ngms_core::generate_recovery_phrase() {
            Ok((phrase, hex_hash)) => {
...
async fn post_migrate(
    ...
    let mut path_mappings: Vec<ngms_migrate::PathMapping> = Vec::new();
    ...
    while let Ok(Some(field)) = multipart.next_field().await {
        ...
        match field_name.as_str() {
            "path_mappings" => {
                match serde_json::from_slice::<Vec<ngms_migrate::PathMapping>>(&data) {
    ...
    match ngms_migrate::run_migration(
        pool,
        ...
        "DiskScan" => {
            ...
            if let Some(series_id) = body.series_id {
                ...
                if let Some((path,)) = series_row {
                    ...
                    match ngms_import::disk_scan(pool, root_path, "series").await {
                        Ok(scan_result) => {
            ...
            let mut total = ngms_import::DiskScanResult {
                files_found: 0,
                files_matched: 0,
                files_unmatched: 0,
