...
use ngms_core::models::{DownloadProtocol, QualityProfile, ReleaseInfo};
...
/// Maps a `ngms_parser::Quality` variant to the core model's discriminant
/// number used in quality profile items.
pub fn parser_quality_to_num(q: ngms_parser::Quality) -> i32 {
    match q {
        ngms_parser::Quality::Unknown => 0,
        ngms_parser::Quality::SDTV => 1,
        ngms_parser::Quality::DVD | ngms_parser::Quality::DVDRip => 2,
        ngms_parser::Quality::WEBDL480p => 3,
        ngms_parser::Quality::WEBRip480p => 4,
        // Bluray480p = 5 in core, but parser doesn't have it
        ngms_parser::Quality::HDTV720p => 6,
        ngms_parser::Quality::WEBDL720p => 7,
        ngms_parser::Quality::WEBRip720p => 8,
        ngms_parser::Quality::Bluray720p => 9,
        ngms_parser::Quality::HDTV1080p => 10,
        ngms_parser::Quality::WEBDL1080p => 11,
        ngms_parser::Quality::WEBRip1080p => 12,
        ngms_parser::Quality::Bluray1080p => 13,
        ngms_parser::Quality::Remux1080p => 14,
        ngms_parser::Quality::HDTV2160p => 15,
        ngms_parser::Quality::WEBDL2160p => 16,
        ngms_parser::Quality::WEBRip2160p => 17,
        ngms_parser::Quality::Bluray2160p => 18,
        ngms_parser::Quality::Remux2160p => 19,
        ngms_parser::Quality::Raw => 20,
    }
}
...
fn parse_quality_num(title: &str) -> i32 {
    let parsed = ngms_parser::parse_release(title);
...
impl DecisionSpecification for LanguageSpec {
    fn is_satisfied(&self, context: &DecisionContext) -> Option<Rejection> {
        ...
        let release_langs = ngms_parser::parse_languages(&context.release.title);
        ...
        let matched = release_langs.iter().any(|lang| {
            *lang == wanted_parser_lang
                || *lang == ngms_parser::Language::Multi
                || *lang == ngms_parser::Language::Unknown
        });
...
fn radarr_id_to_parser_lang(id: i32) -> ngms_parser::Language {
    use ngms_parser::Language;
    match id {
        1 => Language::English,
        2 => Language::French,
        3 => Language::Spanish,
...
fn parser_lang_name(lang: ngms_parser::Language) -> &'static str {
    use ngms_parser::Language;
    match lang {
