    ...
    pub async fn start(self) -> Result<SchedulerHandle> {
            ...
            if enabled.contains(&"plex_integration".to_string()) {
                ...
                join_set.spawn(async move {
                    ...
                    loop {
                        ...
                        let scanner = ngms_plex::PlexScanner::new(plex_recent_pool.clone());
                ...
                join_set.spawn(async move {
                    ...
                    loop {
                        ...
                        let scanner = ngms_plex::PlexScanner::new(plex_full_pool.clone());
                ...
                join_set.spawn(async move {
                    ...
                    loop {
                        ...
                        let sync = ngms_plex::WatchlistSync::new(plex_wl_pool.clone());
                ...
                join_set.spawn(async move {
                    ...
                    loop {
                        ...
                        let refresh = ngms_plex::TokenRefresh::new(plex_token_pool.clone());
                ...
                join_set.spawn(async move {
                    ...
                    loop {
                        ...
                        let sync = ngms_plex::AvailabilitySync::new(avail_pool.clone());
                        if let Err(e) = sync.run().await {
        ...
        // ── Activity / notification cleanup (daily) ─────────────────
        {
            ...
            join_set.spawn(async move {
                ...
                loop {
                    tick.tick().await;
                    let db = ngms_core::Database::from_pool(cleanup_pool.clone());
        ...
        // ── Recycle bin cleanup (every 6 hours) ─────────────────────
        {
            ...
            join_set.spawn(async move {
                ...
                loop {
                    ...
                    match ngms_import::recycle_bin::cleanup_expired_from_config(
                        cleanup_pool.clone(),
                    )
                    .await
                    {
...
async fn import_scan_task(pool: PgPool) -> Result<()> {
    ...
    for (queue_id, media_type, media_id, episode_id, download_id, title, client_id) in &completed {
        ...
        let ctx = ngms_import::ImportContext {
            pool: pool.clone(),
        ...
        match ngms_import::process_completed_download(ctx).await {
            Ok(import_result) => {
...
async fn metadata_refresh_task(pool: PgPool, tmdb_client: Option<Arc<TmdbClient>>) -> Result<()> {
    let refresh_svc = ngms_media::MetadataRefreshService::new(pool.clone());
    ...
    if let Some(ref tmdb) = tmdb_client {
        for series_id in stale_series {
            let svc = ngms_media::SeriesService::new(pool.clone());
    ...
    if let Some(ref tmdb) = tmdb_client {
        for movie_id in stale_movies {
            let svc = ngms_media::MovieService::new(pool.clone());
...
async fn import_list_sync_task(pool: PgPool, tmdb_client: Option<Arc<TmdbClient>>) -> Result<()> {
    ...
    let svc = ngms_media::import_lists::ImportListService::new(pool);
...
async fn scheduled_disk_scan(pool: PgPool) -> Result<()> {
    let db = ngms_core::Database::from_pool(pool.clone());
    ...
    for (i, (path, media_type)) in folders.iter().enumerate() {
        ...
        match ngms_import::disk_scan(&pool, scan_path, media_type).await {
            Ok(result) => {
...
mod tests {
    ...
    /// to `ngms_import::disk_scan`, so "tv" (from user-created
    ...
    async fn test_disk_scan_accepts_all_db_media_types() {
        ...
        for media_type in &["series", "tv", "movie"] {
            let result = ngms_import::disk_scan(&pool, dir.path(), media_type).await;
            if let Err(e) = &result {
                assert!(
