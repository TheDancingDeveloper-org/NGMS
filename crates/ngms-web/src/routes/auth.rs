...
async fn login_inner(
    ...
    let valid = match tokio::task::spawn_blocking(move || {
        ngms_core::auth::verify_password(&password, &hash)
    })
    .await
    {
        Ok(Ok(v)) => v,
    ...
    // Create session
    let token = ngms_core::auth::generate_session_token();
    let token_hash = ngms_core::auth::hash_token(&token);
    ...
    let cookie = format!(
        "ngms_session={token}; HttpOnly; SameSite=Lax; Path=/; Max-Age={}",
        30 * 24 * 60 * 60
    );
...
async fn logout(
    ...
    if let Some(session_token) = extract_session_cookie(&headers) {
        let token_hash = ngms_core::auth::hash_token(session_token);
        let _ = state.db.delete_session(&token_hash).await;
    }

    // Clear cookie
    let cookie = "ngms_session=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0";
...
async fn register(
    ...
    let password_hash = match tokio::task::spawn_blocking(move || {
        ngms_core::auth::hash_password(&password)
    })
    .await
    {
        Ok(Ok(h)) => h,
...
async fn setup(
    ...
    let api_key = ngms_core::auth::generate_session_token();
