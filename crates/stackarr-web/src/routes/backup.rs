...
async fn export_backup(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let resp = BackupResponse {
        ...
        movies: fetch_table!(
            "SELECT id, title, path, quality_profile_id, media_library_folder_id, monitored, tmdb_id, imdb_id, minimum_availability FROM movies ORDER BY id"
        ),
        quality_profiles: fetch_table!(
...
async fn import_restore(
    ...
    if let Some(profiles) = &body.quality_profiles {
        ...
        for p in profiles {
            ...
            let cutoff_format_score =
                i64_field(p, "cutoff_format_score", "cutoffFormatScore", 0) as i32;
            let items = json_field(p, "items", "items");
            let media_type = p
                .get("media_type")
                .or_else(|| p.get("mediaType"))
                .and_then(|v| v.as_str());
    ...
    if let Some(configs) = &body.naming_config {
        ...
        for c in configs {
            ...
            let standard = c
                .get("standard_format")
                .or_else(|| c.get("standardFormat"))
                .and_then(|v| v.as_str());
            let daily = c
                .get("daily_format")
                .or_else(|| c.get("dailyFormat"))
                .and_then(|v| v.as_str());
            let anime = c
                .get("anime_format")
                .or_else(|| c.get("animeFormat"))
                .and_then(|v| v.as_str());
            let season_folder = c
                .get("season_folder_format")
                .or_else(|| c.get("seasonFolderFormat"))
                .and_then(|v| v.as_str());
            let movie = c
                .get("movie_format")
                .or_else(|| c.get("movieFormat"))
                .and_then(|v| v.as_str());
            let movie_folder = c
                .get("movie_folder_format")
                .or_else(|| c.get("movieFolderFormat"))
                .and_then(|v| v.as_str());
