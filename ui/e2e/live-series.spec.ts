...
const BASIC_AUTH = `Basic ${Buffer.from('admin:testpass123').toString('base64')}`

async function login(_page: import('@playwright/test').Page) {
  // Auth is applied via extraHTTPHeaders at the describe level.
  // This stub remains so existing call sites keep working.
}

// ─── Series CRUD via API ───────────────────────────────────

test.describe('Live: Series CRUD (API)', () => {
  test.use({ extraHTTPHeaders: { Authorization: BASIC_AUTH } })
...
test.describe('Live: Wanted', () => {
  ...
  test('J83: wanted page loads with tabs', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Missing', exact: true })).toBeVisible()
    await expect(page.getByRole('button', { name: 'Cutoff Unmet', exact: true })).toBeVisible()
  })

  test('J84: cutoff unmet tab switches', async ({ page }) => {
    await page.goto('/wanted/missing')
    await page.getByRole('button', { name: 'Cutoff Unmet', exact: true }).click()
    await expect(page.getByRole('button', { name: /Search Upgrades/i })).toBeVisible()
  })
})
