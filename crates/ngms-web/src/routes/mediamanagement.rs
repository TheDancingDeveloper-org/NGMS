...
async fn list_recycle_bin(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    match ngms_import::recycle_bin::list_entries(state.db.pool()).await {
...
async fn delete_recycle_entry(
    ...
    match ngms_import::recycle_bin::delete_entry(state.db.pool(), id).await {
...
async fn empty_recycle_bin(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    match ngms_import::recycle_bin::empty_bin(state.db.pool()).await {
