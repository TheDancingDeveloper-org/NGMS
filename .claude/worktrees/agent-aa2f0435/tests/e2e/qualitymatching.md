...
| Search | Reference | StackArr | Titles Matched | Approval Match | Mismatch Rate |
|--------|-----------|----------|----------------|----------------|---------------|
...
All 4 searches fail the 20% approval mismatch threshold. StackArr approves nearly everything; Sonarr/Radarr reject most releases.

## Root Causes

StackArr's decision engine is missing 3 key inputs during release search that Sonarr/Radarr provide:
...
**Top Sonarr/Radarr rejections StackArr misses:**
```
...
StackArr's `QualityAllowedSpec` is not rejecting qualities that are disabled in the profile. The ProfSync UHD profile only allows 2160p qualities, but StackArr approves 1080p, 720p, 480p, and SDTV releases.
...
Radarr rejects releases where the detected language doesn't match the profile. StackArr has no language filtering.
...
StackArr checks queue by download_id (guid) but Sonarr checks by media item — if any release for the same episode is already in queue at a higher quality, it rejects new ones.

## Release Count Differences

| Search | Only in Reference | Only in StackArr |
|--------|-------------------|------------------|
...
"Only in Reference" releases come from torrent indexers that StackArr doesn't search (Prowlarr proxied torrent indexers have no direct API in StackArr). "Only in StackArr" are releases found by StackArr's indexers that Sonarr/Radarr didn't return (timing differences).
...
1. **QualityAllowedSpec** — Parse quality from release title and check against profile items. This is the most fundamental check and should be straightforward since `stackarr-parser` already extracts quality.
