...
StackArr is a unified media management server written in Rust. It manages TV series and movies with embedded torrent (librtbit) and usenet (nzb) download engines, an integrated indexer sidecar (Indexarr), Plex integration, and a React TypeScript frontend.
...
│   ├── stackarr-cardigann/  # Cardigann YAML indexer definition engine
│   ├── stackarr-cardigann-parity/ # QA binary: Cardigann parity testing
...
│   ├── stackarr-migrate/    # Migration tool: import data from legacy media manager databases
...
- **PostgreSQL only.** SQLite is used read-only by the migration tool, and by the standalone `stackarr-bootstrap` binary for its own persistence (`server_names`, `pending_claims` tables).
