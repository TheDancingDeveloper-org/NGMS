...
use ngms_indexer::search::{MovieSearchCriteria, TvSearchCriteria};
use ngms_quality::custom_formats::{CustomFormatDef, CustomFormatEngine};
use ngms_quality::{DecisionContext, DecisionEngine, DownloadDecision, GrabStrategy, rank_releases};
...
fn indexer_to_core(r: ngms_indexer::ReleaseInfo) -> ReleaseInfo {
    ReleaseInfo {
        guid: r.guid,
        title: r.title,
        ...
        protocol: match r.protocol {
            ngms_indexer::newznab::Protocol::Torrent => DownloadProtocol::Torrent,
            ngms_indexer::newznab::Protocol::Usenet => DownloadProtocol::Usenet,
        },
        size: r.size,
        age_days: r.age_days,
...
async fn search_releases(
    ...
    let cf_formats: Vec<CustomFormatDef> = sqlx::query_as::<_, ngms_core::models::CustomFormat>(
...
async fn grab_release(
    ...
    let protocol = match body.protocol.as_deref() {
        Some("torrent") => ngms_download::DownloadProtocol::Torrent,
        _ => ngms_download::DownloadProtocol::Usenet,
    };

    let grab_req = ngms_download::GrabRequest {
    ...
    let core_protocol = match protocol {
        ngms_download::DownloadProtocol::Torrent => "torrent",
        ngms_download::DownloadProtocol::Usenet => "usenet",
    };
